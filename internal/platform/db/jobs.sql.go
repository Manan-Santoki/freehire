...
WITH closed AS (
    ...
    WHERE closed_at IS NULL
      ...
      AND EXISTS (
          ...
          WHERE bh.provider = $1
            AND bh.board = $3
            AND ((bh.last_success_at IS NOT NULL AND bh.last_success_at < now() - $4::interval)
              OR (bh.last_success_at IS NULL AND bh.first_seen_at < now() - $4::interval))
      )
    RETURNING id
), queued AS (
...
func (q *Queries) CloseChronicBoardJobs(ctx context.Context, arg CloseChronicBoardJobsParams) (int64, error) {
	row := q.db.QueryRow(ctx, closeChronicBoardJobs,
		arg.Source,
		arg.BoardPattern,
		arg.Board,
		arg.AgeWindow,
...
WITH closed AS (
    ...
    WHERE closed_at IS NULL
      ...
      AND EXISTS (
          ...
          WHERE bh.provider = $1
            AND bh.board = ''
            AND ((bh.last_success_at IS NOT NULL AND bh.last_success_at < now() - $2::interval)
              OR (bh.last_success_at IS NULL AND bh.first_seen_at < now() - $2::interval))
      )
    RETURNING id
), queued AS (
    INSERT INTO search_delete_outbox (job_id)
...
type CloseChronicProviderJobsParams struct {
	Source    string          `json:"source"`
	AgeWindow pgtype.Interval `json:"age_window"`
}
...
func (q *Queries) CloseChronicProviderJobs(ctx context.Context, arg CloseChronicProviderJobsParams) (int64, error) {
	row := q.db.QueryRow(ctx, closeChronicProviderJobs, arg.Source, arg.AgeWindow)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
type CloseStaleUnseenUnprobeableJobsParams struct {
	Sources    []string           `json:"sources"`
	Cutoff     pgtype.Timestamptz `json:"cutoff"`
	SeenCutoff pgtype.Timestamptz `json:"seen_cutoff"`
}
...
func (q *Queries) CloseStaleUnseenUnprobeableJobs(ctx context.Context, arg CloseStaleUnseenUnprobeableJobsParams) (int64, error) {
	row := q.db.QueryRow(ctx, closeStaleUnseenUnprobeableJobs, arg.Sources, arg.Cutoff, arg.SeenCutoff)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const closeStaleUnsignalledJobs = `-- name: CloseStaleUnsignalledJobs :one
WITH closed AS (
    UPDATE jobs
    SET closed_at     = now(),
        closed_reason = 'expired',
...
func (q *Queries) CloseStaleUnsignalledJobs(ctx context.Context, arg CloseStaleUnsignalledJobsParams) (int64, error) {
	row := q.db.QueryRow(ctx, closeStaleUnsignalledJobs, arg.Sources, arg.Cutoff)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
func (q *Queries) CountChronicBoardJobs(ctx context.Context, arg CountChronicBoardJobsParams) (int64, error) {
	row := q.db.QueryRow(ctx, countChronicBoardJobs,
		arg.Source,
		arg.BoardPattern,
		arg.Board,
		arg.AgeWindow,
...
type CountChronicProviderJobsParams struct {
	Source    string          `json:"source"`
	AgeWindow pgtype.Interval `json:"age_window"`
}
...
SET liveness_strikes = jobs.liveness_strikes + 1,
    ...
    closed_at = CASE
        WHEN jobs.liveness_strikes + 1 >= $1 AND jobs.closed_at IS NULL THEN now()
        ELSE jobs.closed_at
    END,
    closed_reason = CASE
        WHEN jobs.liveness_strikes + 1 >= $1 AND jobs.closed_at IS NULL THEN 'probe_expired'
