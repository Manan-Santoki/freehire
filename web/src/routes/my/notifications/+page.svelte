<script lang="ts">
  ...
  // header bell's most-recent page. Own Paginator rather than the shared
  // notificationCenter singleton (which only ever holds one fixed-size page for
  // the dropdown): this view genuinely pages through everything, so it needs its
  // own item list and offset.
