...
{ "error": "job not found" }`,
  },
  {
    ...
    paragraphs: [
      ...
      'Pagination is bounded on **every** list endpoint: `offset + limit` may not ' +
        'exceed 10000 (`pagination too deep` → 400). This is deep-paging protection, ' +
        'not a cap on the reported total — use filters to narrow rather than paging ' +
        'that far.',
      'The bound is a refusal rather than a clamp, deliberately: a clamped page would ' +
        'answer 200 carrying rows you did not ask for, and a client walking pages would ' +
        'loop over the same page forever without being able to tell. A 400 means ' +
        '"there is no such page", which is the truth.',
      'It applies whichever store answers the endpoint, and it is not the same thing ' +
        'as the rate limit. A rate limit bounds requests per minute; on an endpoint ' +
        'whose `offset` you choose, the work per request is also yours to choose — so a ' +
        'client inside its request budget can still be asking for far more work than ' +
        'the budget implies. Both bounds exist and neither substitutes for the other.',
    ],
  },
  {
    title: 'Errors',
  ...
  "data": {
    ...
    "exact": true
      ...
      },
      {
        ...
        description:
          '`viewed` is the only field here that is not a signed-in count: it is bot-filtered job-page opens by every visitor, signed in or not, so it is orders of magnitude larger than the rest. It is also the only one that is not all-time — `viewed_since` is the first day it covers (null before the first rollup), because the bot-filtered counter shipped without a backfill of the days before it. Every other field is all-time. Aggregate-only — no user identifier is exposed.',
        curl: `curl "${BASE_URL}/stats/engagement"`,
        responseExample: `{ "data": { "saved": 41200, "applied": 18730, "viewed": 903400, "viewed_since": "2026-09-04", "cvs_uploaded": 2140, "cvs_tailored": 860, "match_analyses": 5310, "inboxes_connected": 410, "saved_searches": 1290 } }`,
      },
      {
        method: 'GET',
        path: '/status',
