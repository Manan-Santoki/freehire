...
- [x] 5.3 After deploy, seed the boards with `cmd/add-board`: one boardless `hackernews`
      row, the three GitHub repos, and four `hiringcafe` US keyword boards (software
      engineer, software engineer intern, machine learning engineer, data engineer).
      Onboard further keywords a few at a time — each one's first crawl hydrates its whole
      uncovered slice.
